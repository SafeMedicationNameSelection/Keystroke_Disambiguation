...
The drug product "namespace" for FDA approved drug products and their medication names is a crowded, confusing, constantly changing mashup. Here is a quick tour through some of the many issues that pertain to keeping the drug product and medication names straight. 
...
The second issue is that drug products are often referred to in a shorthand way by using only the generic names of the medications they carry (e.g., “May I have an aspirin?”). The drug product in this case might be an ‘aspirin 81 mg oral tablet’ or an ‘aspirin 325 mg oral tablet’ or something else. Using generic names this shorthand way can cause confusion and may lead to errors. 

The third issue is that multiple medications can be found inside single drug products. These “combination” drug-products are very common and often used. Examples include Vicodin, which contains the generic medications acetaminophen and hydrocodone, and Hyzaar, which contains hydrochlorothiazide and losartan. Combination products are often called by their brand names (e.g., Vicodin and Hyzaar). However, there can be multiple brand names for the same combination and there are also generic combination drug products. For these reasons, combination drug product and medication names are sometimes mixed up.

A fourth issue is like the second. Brand name drug products, whether they are combinations or not, are often referred to in a shorthand way (e.g., “I use Advair to control my asthma.”). Advair is the brand name for a delivery device with an inhalable powder containing salmeterol and fluticasone. Brand names can be problematic when they are similar to other drug product and medication names and also when they are used to designate generic medications that do not carry the original brand name. 

At this point in our tour, we have only covered four issues but perhaps you are beginning to see why people have problems keeping drug product and medication names straight! Unfortunately, there are many more issues.

A fifth issue is that many drug product or generic medication names look-alike. Examples include Actos and Actonel, chlorpromazine and chlorpropamide, prednisone and prednisolone. These and other examples appear on lists of confused names. Sometimes TALL MAN lettering is used to try to distinguish two look-alike names from each other, e.g., prednisONE and prednisOLONE. (However, when disambiguating names by keystrokes, TALL MAN lettering does not help!)
...
A seventh issue has to do with “salt forms.” Chemically speaking, active medication ingredients are sometimes produced using different counterions. When this happens, the generic medication is given two or more similar names, the only difference being the counterion. For example, metoprolol tartrate has a different salt form than metoprolol succinate. Metoprolol tartrate is only used in immediate release oral drug products whereas metoprolol succinate is instead used in sustained release products. A good example of many salt forms comes from calcium, which is provided as an acetate, carbonate, chloride, gluceptate, gluconate, metrizoate, or oxybate. It is easy to confuse salt forms like these. 

An eighth issue is that biosimilar medications have names with matching prefixes and randomly assigned four-letter suffixes. An example of this is Neulasta (pegfilgrastim) and the two biosimilars Fylnetra (pegfilgrastim-pbbk) and Nyvepria (pegfilgrastim-apgf). While having distinguishing suffixes may help support product-specific pharmacovigilance, when substitutions are not wanted and interchangability does not apply, it is easy to see how such similar names can be confused. 

At this point in our tour, we have seen eight issues that can lead to drug product and medication name confusion and mis-selection. There are more issues like these.
